...
- (void)updateUI
{
    ...
    NSUInteger maxScore = resultMax.score;
    NSUInteger minScore = resultMin.score;
    ...
    for (GameResult *result in [[GameResult allGameResults] sortedArrayUsingSelector:self.sortSelector]) {
        NSString *displayString = [NSString stringWithFormat:@"%@ Score: %4lu (%@, %0g)\n", result.gameName, (unsigned long)result.score,

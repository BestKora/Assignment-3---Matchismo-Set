...
-(void)updateCardButton:(UIButton *)cardButton  usingCard:(Card *)card
{
    if ([card isKindOfClass:[SetCard class]]) {
        ...
        [cardButton setAttributedTitle:[self cardAttributedContents:(SetCard *)card]
                              forState:UIControlStateNormal];
        if (card.isChosen) {
            cardButton.backgroundColor = [UIColor colorWithRed: 0.0
                                                         green:0.2
                                                          blue:0.5
                                                         alpha:0.2];
        } else {
...
- (NSAttributedString *)attributedCardsDescription:(NSArray *)cards
{
    ...
    for (Card *card in cards) {
        separator = ([cards indexOfObject:card] ==
                               ([cards count]-1)) ?
...
- (NSAttributedString *)cardAttributedContents:(SetCard *)card
{
    ...
    UIColor *cardColor           = [cardOutlineColor colorWithAlphaComponent:
                                        (CGFloat)[alphaPallette[card.shading] floatValue]];
...
- (NSAttributedString *)textForSingleCard:(Card *)card
{
    NSMutableAttributedString *text =[[NSMutableAttributedString alloc] initWithAttributedString:
                                                    [self cardAttributedContents:(SetCard *)card]];
    [text appendAttributedString:[[NSAttributedString alloc]
                                  initWithString:[NSString stringWithFormat:@"%@",(card.isChosen) ?
